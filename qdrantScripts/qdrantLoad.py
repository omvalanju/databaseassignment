...
for idx, row in df_train.iterrows():
    try:
        ...
        points.append(PointStruct(id=int(row['ecg_id']), vector=vector.tolist(), payload=payload))
    except Exception as e:
        print(f"⚠️ Skipping ID {row['ecg_id']} due to error: {e}")

client.upsert(collection_name=collection_name, points=points)
print(f"✅ Inserted {len(points)} vectors into Qdrant")

# === Query with 2000 test records ===
